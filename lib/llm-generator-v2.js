...
const getOutputCostPerToken = (provider, model) => {
    // Anthropic Claude pricing (per million tokens)
    if (provider === 'anthropic') {
        if (model?.includes('claude-4-sonnet'))
            return 15; // $15 per million output tokens
        if (model?.includes('claude-4-opus'))
            return 75; // $75 per million output tokens
        if (model?.includes('claude-3-7-sonnet'))
            return 15; // $15 per million output tokens
        if (model?.includes('claude-3-5-sonnet'))
            return 15; // $15 per million output tokens
        if (model?.includes('claude-3-5-haiku'))
            return 4; // $4 per million output tokens
        if (model?.includes('claude-3-opus'))
            return 75; // $75 per million output tokens
        if (model?.includes('claude-3-haiku'))
            return 1.25; // $1.25 per million output tokens
        return 15; // Default to Claude 3.7 Sonnet pricing
    }
    // OpenAI pricing (per million tokens)
    if (provider === 'openai') {
        if (model?.includes('gpt-4.1'))
            return 8; // $8 per million output tokens
        if (model?.includes('gpt-4.1-mini'))
            return 1.6; // $1.6 per million output tokens
        if (model?.includes('gpt-4.1-nano'))
            return 0.4; // $0.4 per million output tokens
        if (model?.includes('gpt-4o'))
            return 10; // $10 per million output tokens
        if (model?.includes('gpt-4o-mini'))
            return 0.6; // $0.6 per million output tokens
        if (model?.includes('gpt-4-turbo'))
            return 30; // $30 per million output tokens
        if (model?.includes('gpt-4'))
            return 60; // $60 per million output tokens
        if (model?.includes('gpt-3.5-turbo'))
            return 1.5; // $1.5 per million output tokens
...
export async function generateWithLLM(repoContent, guidelines, outputDir = '.', description, ruleType, outputFormat, options = { provider: 'anthropic' }) {
 ...
 */
function calculateChunkCount(totalTokens, chunkSize) {
 ...
 */
async function* chunkIterator(text, chunkSize, options) {
    console.log(pc.cyan('\n┌─────────────────────────────────────────┐'));
...
async function generateWithProvider(repoContent, guidelines, outputDir = '.', description, ruleType, outputFormat, options = { provider: 'anthropic' }) {
...
//# sourceMappingURL=llm-generator-v2.js.map