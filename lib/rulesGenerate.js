...
export async function rulesGenerate(repoPath, options) {
    try {
        ...
        try {
            // Try with module path first (for global installations)
            ...
            // If not found in module, try with a local path (for development)
            try {
        ...
        const generatedRules = await generateWithLLM(repoText, guidelinesText, outputDir, options.description, options.ruleType, options.outputFormat, {
...
async function runRepomix(repoPath, outputDir, additionalOptions) {
    try {
        ...
        // Add options to help with special token issues
        command += ' --remove-comments --remove-empty-lines';
...
async function readRepomixFile(filePath) {
    try {
...
async function readGuidelines(guidelinesPath) {
    try {
...
function generateMockRepoContent(repoName) {
...
function generateMockGuidelines() {
...
//# sourceMappingURL=rulesGenerate.js.map