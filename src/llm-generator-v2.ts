...
// Cost per token for different providers and models (per million tokens) - INPUT COSTS
const getCostPerToken = (provider: string, model?: string): number => {
  // Anthropic Claude pricing (per million tokens)
  if (provider === 'anthropic') {
    if (model?.includes('claude-4-sonnet')) return 3; // $3 per million input tokens
    if (model?.includes('claude-4-opus')) return 15; // $15 per million input tokens
    if (model?.includes('claude-3-7-sonnet')) return 3; // $3 per million input tokens
    if (model?.includes('claude-3-5-sonnet')) return 3; // $3 per million input tokens
    if (model?.includes('claude-3-5-haiku')) return 0.8; // $0.8 per million input tokens
    if (model?.includes('claude-3-opus')) return 15; // $15 per million input tokens
    if (model?.includes('claude-3-haiku')) return 0.25; // $0.25 per million input tokens
    return 3; // Default to Claude 3.7 Sonnet pricing
  }
  
  // OpenAI pricing (per million tokens)
  if (provider === 'openai') {
    if (model?.includes('gpt-4.1')) return 2; // $2 per million input tokens
    if (model?.includes('gpt-4.1-mini')) return 0.4; // $0.4 per million input tokens
    if (model?.includes('gpt-4.1-nano')) return 0.1; // $0.1 per million input tokens
    if (model?.includes('gpt-4o')) return 2.5; // $2.5 per million input tokens
    if (model?.includes('gpt-4o-mini')) return 0.15; // $0.15 per million input tokens
    if (model?.includes('gpt-4-turbo')) return 10; // $10 per million input tokens
    if (model?.includes('gpt-4')) return 30; // $30 per million input tokens
    if (model?.includes('gpt-3.5-turbo')) return 0.5; // $0.5 per million input tokens
    return 2.5; // Default to GPT-4o pricing
  }
  
  // AWS Bedrock pricing (same as Anthropic for Claude models)
  if (provider === 'bedrock') {
    if (model?.includes('claude-4-sonnet')) return 3;
    if (model?.includes('claude-4-opus')) return 15;
    if (model?.includes('claude-3-7-sonnet')) return 3;
    if (model?.includes('claude-3-5-sonnet')) return 3;
    if (model?.includes('claude-3-5-haiku')) return 0.8;
    if (model?.includes('claude-3-opus')) return 15;
    if (model?.includes('claude-3-haiku')) return 0.25;
    return 3; // Default to Claude pricing
  }
...
// Cost per token for different providers and models (per million tokens) - OUTPUT COSTS
const getOutputCostPerToken = (provider: string, model?: string): number => {
  // Anthropic Claude pricing (per million tokens)
  if (provider === 'anthropic') {
    if (model?.includes('claude-4-sonnet')) return 15; // $15 per million output tokens
    if (model?.includes('claude-4-opus')) return 75; // $75 per million output tokens
    if (model?.includes('claude-3-7-sonnet')) return 15; // $15 per million output tokens
    if (model?.includes('claude-3-5-sonnet')) return 15; // $15 per million output tokens
    if (model?.includes('claude-3-5-haiku')) return 4; // $4 per million output tokens
    if (model?.includes('claude-3-opus')) return 75; // $75 per million output tokens
    if (model?.includes('claude-3-haiku')) return 1.25; // $1.25 per million output tokens
    return 15; // Default to Claude 3.7 Sonnet pricing
  }
  
  // OpenAI pricing (per million tokens)
  if (provider === 'openai') {
    if (model?.includes('gpt-4.1')) return 8; // $8 per million output tokens
    if (model?.includes('gpt-4.1-mini')) return 1.6; // $1.6 per million output tokens
    if (model?.includes('gpt-4.1-nano')) return 0.4; // $0.4 per million output tokens
    if (model?.includes('gpt-4o')) return 10; // $10 per million output tokens
    if (model?.includes('gpt-4o-mini')) return 0.6; // $0.6 per million output tokens
    if (model?.includes('gpt-4-turbo')) return 30; // $30 per million output tokens
    if (model?.includes('gpt-4')) return 60; // $60 per million output tokens
    if (model?.includes('gpt-3.5-turbo')) return 1.5; // $1.5 per million output tokens
    return 10; // Default to GPT-4o pricing
  }
  
  // AWS Bedrock pricing (same as Anthropic for Claude models)
  if (provider === 'bedrock') {
    if (model?.includes('claude-4-sonnet')) return 15;
    if (model?.includes('claude-4-opus')) return 75;
    if (model?.includes('claude-3-7-sonnet')) return 15;
    if (model?.includes('claude-3-5-sonnet')) return 15;
    if (model?.includes('claude-3-5-haiku')) return 4;
    if (model?.includes('claude-3-opus')) return 75;
    if (model?.includes('claude-3-haiku')) return 1.25;
    return 15; // Default to Claude pricing
  }
 ...
 */
async function* chunkIterator(text: string, chunkSize: number | undefined, options: LLMGeneratorOptions): AsyncGenerator<{
  chunk: string;
  index: number;
  tokenCount: number;
  totalChunks: number;
  ...
  const inputCostPerMillionTokens = getCostPerToken(options.provider, options.model);
  const outputCostPerMillionTokens = getOutputCostPerToken(options.provider, options.model);
  
  const inputCostPerToken = inputCostPerMillionTokens / 1000000; // Convert from per-million to per-token
  const outputCostPerToken = outputCostPerMillionTokens / 1000000; // Convert from per-million to per-token
  
  // Estimate output tokens as same as input (conservative estimate)
  const estimatedOutputTokens = totalTokens;
  
  const inputCost = totalTokens * inputCostPerToken;
  const outputCost = estimatedOutputTokens * outputCostPerToken;
  const totalEstimatedCost = inputCost + outputCost;
  
  const costDisplay = inputCostPerMillionTokens === 0 ? 'free' : `$${inputCostPerMillionTokens}/MTok input, $${outputCostPerMillionTokens}/MTok output`;
  console.log(pc.yellow(`● Estimated total cost: $${totalEstimatedCost.toFixed(4)} (${formatTokenCount(totalTokens)} input + ${formatTokenCount(estimatedOutputTokens)} output tokens × ${costDisplay})`));
...
async function generateWithProvider(
  ...
  const chunkGen = chunkIterator(repoContent, options.chunkSize, options);
